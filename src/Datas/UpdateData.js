const updateDatas = [
    ...
    {title: '사운드 폰트를 이용한 음악교실 오픈', date:'2024-08-08', auth:'pukkok', file: ''},
    {title: 'vexflow를 이용한 악보제작', date:'2024-08-13', auth:'pukkok', file: ''},
...
`
안녕하세요 푹곡입니다.
오늘의 업데이트 내용은 soundfont를 이용하여 피아노 기능을 구현했습니다.

악기가 없더라도 음악 수업을 할 수 있으면 어떨까? 하는 생각으로 구현했습니다.

처음에 피아노를 만들때, 하나하나의 피아노 음원을 다운받아서,
버튼 클릭시 Audio를 생성하여 play하는 방법을 사용하였으나,
이 경우 파일들을 브라우저가 가지고 있어야 하며,
건반의 노트 하나 하나마다의 음원을 구하기가 어려웠습니다.

혹시 라이브러리가 있는지 찾아보던 중
react-piano라는 라이브러리를 발견했는데, 
여기선 음원은 따로 준비해야 한다고 적혀있었습니다.

음원파일을 찾기위해 소스 코드를 살펴본 결과
음원파일을 사용하지 않고 soundfont라는 라이브러리를 사용하는 것을 발견했습니다.
리액트에선 soundfont-player과 audio-context를 이용하여 음원 파일 없이도, 피아노를 구현할 수 있었습니다.
`,
// vex flow
`
안녕하세요 푹곡입니다.
오늘의 업데이트 내용은 vexflow를 이용한 악보제작입니다.
이번 라이브러리를 이용한 악보 제작과정에 많은 어려움이 있었는데요.

이번 vexflow 라이브러리를 사용하게된 계기는 유치원 악보들을 다운받아 사용하려하니,
생각보다 화질이 좋지 않았습니다. 그래서 악보를 제작해서 사용해야겠다 라는 생각을 했으며,
이번에도 이미지들을 받아서 악보제작을 하려 했으나, 악보 제작에 관한 라이브러리를 찾아보니
vexflow라는 라이브러리가 있었습니다.
이 라이브러리는 악보의 한마디 마다 svg로 만들어서 사용하는 라이브러리였습니다.

그러나, 생각보다 상세한 내용에 대해서는 docs가 정확하지 않아서 사용하기 어려웠습니다.

처음 easyscore와 간단한 것들은 예시가 있어서 따라서 사용하기 편했으나, 정말 간단한 예시만 있어서,
악보를 제작하기 위해서는 이외 도돌이표, 볼타, 점음표, 쉼표, 가사, 숨표 등 여러가지 요소들이 필요했습니다.
또 easyscore과 기본적으로 사용하는 악보제작 방식의 툴 사용방법이 달라서 헷갈리는 부분도 많았습니다.

도돌이표와 마침표 부분은 docs를 찾아보니 endbar라인을 변경하는 부분을 찾아서 변경하는 방법이 있었고,

볼타 도돌이표 1번, 2번 같이 넘어가는 곳을 표현한 것으로 제작시 따로 만들어서 추가하는 방법을 사용했습니다.

이 라이브러리는 악보를 제작할 때 마디 마디 마다 따로따로 제작을 하는데,
타이 같은 경우는 시작 부분과 끝부분을 연결할때 다음 마디로 넘어간 경우 어떻게 연결해야하는지 몰랐습니다.
일단 시작하는 인덱스와 끝나는 인덱스가 필요했는데 여러가지 테스트를 하면서
처음 인덱스만 있는 경우를 살펴보니 다음 마디 부분까지 쭉 이어졌습니다.
현재 forEach를 사용하기 때문에 이전 마디와 타이인덱스를 변수에 저장하여 사용하여,
타이를 완성했습니다.

docs를 찾아보기 너무 어려워서 챗GPT의 도움을 좀 빌렸으나, 챗GPT 친구도 DOCS가 어려웠나봅니다..
알려준 예시 코드를 이용했으나 아직 오류가 많아 아쉬웠습니다.

자세한 내용이 필요하다면 https://vexflow.com 에서 확인하세요!
